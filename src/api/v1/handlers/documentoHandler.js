const {
getDocumento, getDocumentoOpcional
...
const getDocumentoOpcionalHandler=async (req,res) => {
    const {id}=req.params
    try {
        const response=await getDocumentoOpcional(id);
       if(!response){
        return res.status(201).json({message:"Error en el handler ", data:[]})
       }
       return res.status(200).json({message:"Correcto Docs ", data:response})
