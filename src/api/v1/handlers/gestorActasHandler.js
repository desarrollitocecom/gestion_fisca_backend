...
const { ControlActa, Usuario, TramiteInspector, RangoActa } = require('../../../config/db_connection');
...
const createControlActaHandler = async (req, res) => {
  const { numero_actas, id_inspector, observaciones_inicio, id_encargadoInicio, actas, anio } = req.body;

  console.log("numero_actas", numero_actas,
    "id_inspector", id_inspector,
    "observaciones_inicio", observaciones_inicio,
    "id_encargadoInicio", id_encargadoInicio,
    "actas", actas,
    "anio", anio);
  ...
  if (!actas || !Array.isArray(actas)) {
    errores.push("El campo 'actas' es obligatorio y debe ser un array.");
  }
  ...
  try {
    // Crear el nuevo Control de Acta
    ...
    if (!newControlActa) {
      return res.status(201).json({
        message: 'Error al crear el Control de Acta',
        data: []
      });
    }
    ...
    // Transformar los datos del array 'actas' concatenando el año
    const actasConRangoId = actas.map(acta => ({
      numero_acta: `${acta.numero_acta}-${anio}`, // Concatenar el número del acta con el año
      id_inspector,
      fecha,
    ...
    // Insertar las actas en la base de datos
    await ControlActa.bulkCreate(actasConRangoId);

    return res.status(200).json({
      message: 'Se creó el control de Acta correctamente',
      data: newControlActa
    });
