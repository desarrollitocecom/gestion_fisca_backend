const { RSA, Usuario, NC, TramiteInspector, DescargoRSA } = require('../../../config/db_connection'); // Asegúrate de que la ruta al modelo sea correcta
const { saveImage, deleteFile } = require('../../../utils/fileUtils')
const { Sequelize } = require('sequelize');
const myCache = require("../../../middlewares/cacheNodeStocked");

const createRSAController = async ({ nro_rsa, fecha_rsa, fecha_notificacion, documento_RSA, id_descargo_RSA, id_nc, id_AR2 }) => {
  let documento_path;
  try {
...
const getAllRSAforPlataformaController = async () => {
  try {
    const response = await RSA.findAll({
      attributes: [
        [Sequelize.col('RSA.id'), 'id'],
        [Sequelize.col('RSA.usuario'), 'usuario'],
        'createdAt',
      ],
    });
    return response || null;
